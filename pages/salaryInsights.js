...
export default class SalaryInsights {
    constructor(page) {
        ...
        this.promoSectionDescription = this.promoSection.locator('.MuiGrid-root.MuiGrid-item.css-tc4co3');
    }
    ...
    async selectCountryOptionFromDropdown(country) {
        ...
        await this.page.getByRole('listbox').locator('div').filter({ hasText: country }).nth(1).click();
    }


    async verifyFilterBarForRoleCountryAndCurrencyCode(role, country, currencyCode) {
    ...
    async verifySalaryTableForRoleCountryAndCurrencySymbol(role, country, currencySymbol) {
        await expect(this.salaryTable).toContainText(`Senior ${role} compensation in ${country}`);
        await expect(this.salaryTable).toContainText(currencySymbol);
    }


    async verifyPromoSectionForRoleCountryAndCurrencySymbol(role, country, currencySymbol) {
        await expect(this.promoSection).toContainText(`How much does a Senior ${role} make in ${country}?`);

        const promoDescriptionText = await this.getCleanedText(this.promoSectionDescription);

        // Assert all parts of the promo section description
        await expect(promoDescriptionText).toContain(`The median salary is ${currencySymbol}`);
        await expect(promoDescriptionText).toContain(`per year for a Senior ${role} in ${country}`);
        await expect(promoDescriptionText).toContain(`Salary estimates are based on anonymous submissions to Deel by ${role} employees in ${country}`);

        // Assert country flag
        const countryFlag = this.promoSection.locator(`img[alt="${country.toLowerCase()}"]`);
